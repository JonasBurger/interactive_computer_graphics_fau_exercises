...
vec2 reflection(vec3 facet_normal_view_space)
{
    ...
    vec3 reflection_vector_view_space = reflect(-camera_location_vector, facet_normal_view_space);
...
void main()
{
    ...
    for(int i = 0; i < glossyRays; ++i){
        ...
        float z0 = sqrt(-2.f*log(u.x))*cos(2.f*PI*u.y);
        float z1 = sqrt(-2.f*log(u.x))*sin(2.f*PI*u.y);
